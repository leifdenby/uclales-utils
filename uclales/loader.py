...
class UCLALES_NetCDFHandler():
    ...
    def get_data_and_grid(self, var_name, timestep):
        ...
        if var_name in ['rho', 'T', 'q_v', 'q_l']:
            ...
            if 1000*r_t.max() < 1.0:
                warning.warn("The `r_t` may actually be in g/kg, but we're assuming that the bug in UCLALES where the mixing ratios are mislabelled")
            q_t = r_t/(r_t + 1.)
            q_l = r_l/(r_l + 1.)
            q_r = r_r/(r_r + 1.)
            q_i = r_i/(r_i + 1.)
    ...
    @property
    def variables(self):
        return self.fhandle.variables.keys() + ['rho', 'T', 'q_v', 'q_l', ]
