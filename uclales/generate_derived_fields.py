...
# from cloud_tracking_analysis import utils
...
def run(k_start, k_end):
    ...
    for k in range(k_start, k_end):
        ...
        if not os.path.exists(temperature_filename):
            ...
            copy_vars = ['time', 'zt', ]
            T_var__handle = fh_T.createVariable(name="T", type="float", dimensions=fh_base.variables['t'].dimensions)
        ...
        if not os.path.exists(density_filename):
            ...
            copy_vars = ['time', 'zt', ]
            rho_var__handle = fh_rho.createVariable(name="rho", type="float", dimensions=fh_base.variables['t'].dimensions)
        ...
        for var_name in vars:
            for fh in f_handles:
                ...
                if not var_name in fh.variables:
                    var_handle__new = fh.createVariable(name=var_name, type=var_handle.typecode(), dimensions=(var_name,))
                else:
        ...
        if add_density_info or add_temperature_info:
            for tn in timesteps:
                ...
                def get_data(var_name, expected_units):
                    ...
                    if not var_name in fhs:
                        fhs[var_name] = scipy.io.netcdf_file(fname_base.format(k=k, var=var_name))
                ...
                for i in range(Nx):
                    ...
                    if add_density_info:
                        ...
                        rho = UCLALES_NetCDFHandler.calc_density(q_d=q_d[s], q_v=q_v[s], q_l=q_l[s], q_r=q_r[s], q_i=q_i[s], T=T, p=p[s])
